...
def future_logits(logits, curtime, shouldSampleInTheFuture):
    """ don't sample events in the past """
    if curtime > 0:
        logits[TIME_OFFSET:TIME_OFFSET+curtime] = -float('inf')
        
    if shouldSampleInTheFuture:
        logits[TIME_OFFSET:TIME_OFFSET+curtime+10] = -float('inf')
...
def add_token(model, z, tokens, top_p, current_time, debug=False, active_instruments=None, forceTime=None, forceDuration=None, monophony=False):
    ...
    with torch.no_grad():
        for i in range(3):
            ...
            if i == 0:
                logits = future_logits(logits, current_time - offset, len(tokens) > 0 and monophony)
            elif i == 1:
            ...
            if i == 0 and forceTime is not None:
                token = forceTime - offset
            if i == 1 and forceDuration is not None:
                token = forceDuration
...
def generate(model, start_time, end_time, inputs=None, controls=None, top_p=1.0, debug=False, delta=DELTA*TIME_RESOLUTION, active_instruments=None, monophony=False):
    if inputs is None:
        inputs = []
    ...
    with tqdm(range(end_time-start_time)) as progress:
        ...
        while True:
            ...
            new_token = add_token(model, z, tokens, top_p, max(start_time,current_time), active_instruments=active_instruments, monophony=monophony)
